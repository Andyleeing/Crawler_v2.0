...
public class MovieFuction {
	...
	public MovieFuction(JDBCConnection jdbc) {
		this.jdbc = jdbc;
	}
	public int infoCrawler(String url,long time,String date) {
		...
		if (movieinfoContent != null && movieinfoContent.length() > 100) {
			CrawlerThread.saveData("youku info movie "+rowKey+" "+url+" "+date.replaceAll(" ","_"), movieinfoContent);
	...
	public int playCrawler(String url,long time,String date) {
		...
		if (Content != null && Content.length() > 100) {
			...
			if(index > -1 && index1 > -1)
			{
				...
				String sumplayCountURL = "http://v.youku.com/QVideo/~ajax/getVideoPlayInfo?_rt=1&_ro=&id=" + videoid + "&sid=" + showid + "&type=vv&catid=96";
				...
				String relate = "http://ykrec.youku.com/show/packed/list.json?vid="
					+ videoid + "&sid=" + showid
					+ "&cate=96&apptype=1&pg=3&module=1&pl=6";
				String relateContent = Function.visitURL(relate);
				Content += "*$$$*" + relateContent;
			}
		}
		if (Content != null && Content.length() > 100) {
			CrawlerThread.saveData("youku play movie "+playRow+" "+url+" "+date.replaceAll(" ","_"), Content);
			return 1;
		}
