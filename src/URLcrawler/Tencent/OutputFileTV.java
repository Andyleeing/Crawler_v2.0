...
public class OutputFileTV {
	HBaseCRUD hbase;

	OutputFileTV(HBaseCRUD hbase) {
		this.hbase = hbase;
	}
	...
	public void tencentOutput(String strUrl, String sqlTableName) {
		try {
			...
			for (Element link : allLink) {
				...
				String save = temp + " + " + score + " * type:tv"; // save是加上换行符的url
				...
				// "D://test/url/tvUrl.txt", true);
				// file.write(save.getBytes());
				Document doc1 = ceshi.getdoc(temp);
				// FileOutputStream file2 = new
				// FileOutputStream("D://test/url/tvPlay.txt",true);
				Elements allLink2 = null;
				try {
					allLink2 = doc1.getElementsByClass("album_link");
				} catch (Exception e) {
					// e.printStackTrace();
				}
