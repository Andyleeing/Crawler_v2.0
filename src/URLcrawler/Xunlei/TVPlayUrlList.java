...
public class TVPlayUrlList implements Runnable{
	...
	public static ArrayList<TVPlayUrlList> pool;
	public JDBCConnection jdbconn = new JDBCConnection();
	
	public TVPlayUrlList(long time, String date) {
		this.time = time;
		this.date = date;
	}
...
//			OutputStream out = new FileOutputStream(f,true);
			...
			int page = 0;
			if(doc != null){
				featureContent = doc.getElementById("fenji_"+ Integer.toString(page) +"_asc");
			}
...
//				System.out.println(strUrl);
			...
			{
				allLink = featureContent.getElementsByClass("pic").select("a[href]");//allLink中存储每页中各个电影的链接。
					...
					if (link != null) {
						...
						if(vipIndex != 7){
							...
							if(map.containsKey(movieid +"-" +subid) == false){
								...
								save(newUrl + "@"+ "<*TY->电视剧<-TY*>" + "<*ST->" + subTitle + "<-ST*>" );
