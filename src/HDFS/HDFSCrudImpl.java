...
public class HDFSCrudImpl implements IHDFSCrud {
	...
	public boolean create(String content, String filePath) {
		...
		while (num < 5) {
			...
			try {
				...
				content = null;
				//synchronized (count) {
					System.out.println("hdfs save file"+filePath);
				//}
				break;
			} catch (IOException e) {
				//e.printStackTrace();
	...
	public boolean readHDFSListAll(String name, String path) {
		...
		try {
			...
			for (int i = 0; i < status.length; i++) {
				FileStatus temp[] = hdfs.listStatus(new Path(status[i]
						.getPath().toString()));
				for (int k = 0; k < temp.length; i++) {
					System.out.println("file path = "
							+ temp[k].getPath().toString());
