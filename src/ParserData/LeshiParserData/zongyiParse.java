...
public class zongyiParse {
	
	public HBaseCRUD hbase;
	public JDBCConnection jdbconn;

	public zongyiParse(HBaseCRUD hbase, JDBCConnection jdbconn) {
		this.hbase = hbase;
		this.jdbconn = jdbconn;
	}

	public String VarietyName = "无";// 综艺名
	public String VarietyTime = "无";
	public String VarietyStation = "无";// 电视台
	public String VarietyHost = "无";// 主持人
	public String VarietyType = "";// 类型
	public String VarietyGuest = "";// 嘉宾
	public String Intro = "无";// 简介
	public String PlayCount = "0";
	public String comCount = "0";
	public String sumComCount = "0";
	public String Grade = "0";
	public String sumPlayCount = "0";
	...
	public void parser(String details, String rowkey, String typeString,
		...
		String url = "http://www.letv.com/zongyi/" + rowkey + ".html";
		...
		if (details.contains("play http")) {
			...
			int Year = infoDetails.indexOf("期数");
			if (Year > 0) {
				int YearBegin = infoDetails.indexOf("<em>", Year);
				if (Year > 0 && YearBegin > 0)
					VarietyTime = infoDetails.substring(Year + 8, YearBegin);
			}

			int Station = infoDetails.indexOf("播出");
			if (Station > 0) {
				int staBegin = infoDetails.indexOf("_blank", Station);
				int staEnd = infoDetails.indexOf("</a>", staBegin);
				if (staBegin > 0 && staEnd > 0)
					VarietyStation = infoDetails
							.substring(staBegin + 8, staEnd);
			}

			int type = infoDetails.indexOf("类型");

			VarietyType = Type(infoDetails, type);

			int Host = infoDetails.indexOf("主持人", type);
			if (Host > 0) {
				int hostBegin = infoDetails.indexOf("_blank", Host);
				int hostEnd = infoDetails.indexOf("</a>", hostBegin);
				if (hostBegin > 0 && hostEnd > 0)
					VarietyHost = infoDetails.substring(hostBegin + 8, hostEnd);
			}
			int Guest = infoDetails.indexOf("嘉宾");
			...
			if(IntroEnd>end){
				Intro = infoDetails.substring(IntroIndex + 11, infoDetails.indexOf("</p>",IntroIndex));
			}
			else{
			...
			if (Guest > 0) {
				int max = infoDetails.indexOf("class=", Guest);
				if (Guest < max)
					VarietyGuest = MovieActor(infoDetails.substring(Guest, max));
			}
		}
		...
		if (flag == 1) {
			...
			values = new String[] { rowkey, VarietyTime, "ls", Name,
					PictureURL, VarietyStation, VarietyType, VarietyHost,
					VarietyGuest.toString(), typeString, Intro, url };
			...
			values = new String[] { rowkey, key, "ls", VarietyName, playurl,
					Duration,album };

			hbase.putRows("videoinfo", rows, colfams, quals, values);
			hbase.putRows("videoinfobak", rows, colfams, quals, values);

			rows = new String[] { key4, key4, key4, key4, key4, key4, key4, key4 };
		...
		VarietyName = null;// 综艺名
		VarietyTime = null;
		VarietyStation = null;// 电视台
		VarietyHost = null;// 主持人
		VarietyType = null;// 类型
		VarietyGuest = null;// 嘉宾
		Intro = null;// 简介
		PlayCount = null;
		comCount = null;
		sumComCount = null;
	...
	public void infoDataToMysql(String rowkey,String time,String date) {
		...
		if(VarietyTime.contains("-")){
			VarietyTime = VarietyTime.substring(0, 4);
			yearInt = Integer.valueOf(VarietyTime);
		}
		if(yearInt==0){
			yearInt=-1;
		}
		...
		directorValue.text = "host1";
		directorValue.value = VarietyHost;
		...
		String[] typeSplits = VarietyType.split("@");
		String[] actorSplits = VarietyGuest.split("@");
		String temp = "";
		...
		MovieNameValue.value = VarietyName;
		...
		areaValue.text = "area";
		areaValue.value = VarietyStation;
	...
	public String MovieActor(String details) {
		StringBuilder MovieActor = new StringBuilder();
		int flag = details.indexOf("title=\"");
		while (flag > 0) {
			int ActorBegin = details.indexOf("title=\"");
			if (ActorBegin < 0)
				break;
			int ActorEnd = details.indexOf("\"", ActorBegin + 7);
			String ActorName = "";
			if (ActorBegin < 0 || ActorEnd < 0)
				break;
			else
				ActorName = details.substring(ActorBegin + 7, ActorEnd);
			MovieActor.append("@");
			MovieActor.append(ActorName);
			details = details.substring(ActorEnd);
			flag = details.indexOf("title=\"");
		}
		return MovieActor.toString();
	}
}
