...
public class movieParse {
	...
	public String MovieName = "";// 电影或者电视剧名称
	public String MovieYear = "";// 年份
	public String MovieCountry = "";// 出产地
	public String MovieDirector = "";// 导演
	...
	public movieParse(HBaseCRUD hbase, JDBCConnection jdbconn) {
		this.hbase = hbase;
	...
	public void parser(String details, String rowkey, String typeString,
		...
		String url = "http://www.letv.com/movie/" + rowkey + ".html";
		...
		if (details.contains("play http")) {
			...
			int Year = infoDetails.indexOf("上映日期");
			if (Year > 0) {
				int yearBegin = infoDetails.indexOf("<em>", Year);
				int yearEnd = infoDetails.indexOf("</em>", yearBegin);
				if (yearBegin > 0 && yearEnd > 0)
					MovieYear = infoDetails.substring(yearBegin + 4, yearEnd);
			}

			int type = infoDetails.indexOf("类型");
			MovieType = Type(infoDetails, type);

			int Director = infoDetails.indexOf("导演", type);
			...
			MovieActor = ActorScore(infoDetails);

			int Country = infoDetails.indexOf("国家地区", Director);
		...
		if (flag == 1) {
			if (MovieActor == null){
				jdbconn.log("徐萌", rowkey+"+ls", 1, "ls", playurl, "无演员数据", 2);
				return;
			}
	...
	public String Type(String details, int type) {
		...
		int typeli = details.indexOf("class=\"li_06", type);
		if (type < 0 || typeli < 0)
			return "";
		else
