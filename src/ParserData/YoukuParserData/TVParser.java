...
public class TVParser {
	...
	public TVParser(HBaseCRUD hbase,JDBCConnection conn) {
		this.hbase = hbase;
		this.conn = conn;
	}
	...
	public void playParser(String source, String key, String infoRowKey,
		...
		if (source == null || source.length() < 20) {
			//add by hanjiaxing 
			conn.log("hanjiangxue", key, 1, "yk", "", "TV movie source null", 2);
			return;
		}
	...
	public void infoParser(String source, String key, String url,
		...
		if (source == null || source.length() < 20) {
			//add by hanjiaxing 
			conn.log("hanjiangxue", key, 1, "yk", "", "tv video source null", 2);
			return;
		}
		String mykey = key;
		String moviename = "";
		...
		int indexytime = source.indexOf("<label>优酷开播:</label>");
		if (indexytime > 0) {
		...
		if (indexarea > 0) {
			...
			String con = "http://www.youku.com/v_olist/c_97_a_";
			String endcon = ".html";
			int indexa = 0;
			int endindexa = 0;
		...
		if (indextype > 0) {
			...
			if (indexar >= 0 && endAre >= 0 && endAre > indexar) {
				String sourceare = contmp.substring(indexar, endAre); // sourceare是国家。
				String con = "http://www.youku.com/v_olist/c_97_g_";
				String endcon = ".html";
				int indexa = 0;
				int endindexa = 0;
		...
		if (indexus > 0) {
			...
			if (indexar1 >= 0 && endAre1 >= 0) {
				String sourcepar = contmp1.substring(indexar1, endAre1); // sourcepar是演员信息。
				int indexur = 0;
				int endur = 0;
		...
		quals = new String[] { "inforowkey", "year", "website", "name",
				...
				"type", "director", "mainactor", "actorlist", "price",
