...
public class ZYParser {

	public HBaseCRUD hbase;
	public JDBCConnection conn;

	public ZYParser(HBaseCRUD hbase,JDBCConnection conn) {
		this.hbase = hbase;
		this.conn = conn;
	}
	...
	public void playParser(String source, String key, String infoRowKey,
		...
		if (source == null || source.length() < 20) {
			//add by hanjiaxing 
			conn.log("hanjiangxue", key, 1, "yk", "", "zongyi movie source null", 2);
			return;
		}
	...
	public void infoParser(String source, String key, String url,
		...
		if (source == null || source.length() < 20) {
			//add by hanjiaxing 
			conn.log("hanjiangxue", key, 1, "yk", "", "zongyi video source null", 2);
			return;
		}
		String mykey = key;
		String moviename = "";
		...
		if (indexarea > 0) {
			...
			String con = "http://www.youku.com/v_olist/c_85_a_";
			String endcon = ".html";
			int indexa = 0;
			int endindexa = 0;
		...
		if (indextype > 0) {
			...
			if(indexar >= 0 && endAre >= 0 && endAre > indexar) {
			String sourceare = contmp.substring(indexar, endAre); // sourceare是国家。
			String con = "http://www.youku.com/v_olist/c_85_g_";
			String endcon = ".html";
			int indexa = 0;
			int endindexa = 0;
		...
		values = new String[] { urlkey, movieyear, "yk", moviename, jpgurl,alias,Time,YTime,Lan,Are,Type,Director,Actor,Actorprod,price,"zongyi",detail,award,url};
		try {
		...
		values = new String[] { urlkey, movieyear, "yk","n",timestampkey, moviename, score,douscore,dingcai,sum,dura,totalNum,shou,comment,incresum,"zongyi",free+""};
		try {
