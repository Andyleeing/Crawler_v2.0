...
public class Parser {
	...
	public Parser(HBaseCRUD hbase,JDBCConnection conn) {
		this.hbase = hbase;
		this.conn = conn;
	}
	...
	public void playParser(String source, String key, String infoRowKey,
		...
		if (source == null || source.length() < 20) {
			//add by hanjiaxing 
			conn.log("hanjiangxue", key, 1, "yk", "", "movie movie source null", 2);
			return;
		}
		...
		String key2 = infoRowKey + "+" + urlkey + "+" + "yk" + "+" + "n" + "+"
				+ timestampkey;
		rows = new String[] { key1, key1, key1, key1, key1,key1,key1};
		colfams = new String[] { "R", "R", "R", "B", "B","B","C" };
		quals = new String[] { "inforowkey", "playrowkey", "website", "name",
				"url","showtype","comment" };
		values = new String[] { infoRowKey, urlkey, "yk", othername, url ,showtype,comment};
		try {
		...
		quals = new String[] { "inforowkey", "playrowkey", "website", "flag",
				"timestamp", "recommend", "related", "updown", "sumplaycount"};
		values = new String[] { infoRowKey, urlkey, "yk", "n", timestampkey,
				recommend, Dramavideo, upVideoTimes + "@" + downVideoTimes,
				playcount };
		try {
	...
	public void infoParser(String source, String key, String url,
		...
		if (source == null || source.length() < 20) {
			//add by hanjiaxing 
			conn.log("hanjiangxue", key, 1, "yk", "", "movie video source null", 2);
			return;
		}
		String mykey = key;
		String moviename = "";
		...
		if (indexsc >= 0) {
			...
			if (indexdingcai > 0) {
				int enddingcai = source.indexOf(">", indexdingcai);
				if (enddingcai > 0)
		...
		if (indexarea > 0) {
			...
			String con = "http://www.youku.com/v_olist/c_96_a_";
			String endcon = ".html";
			int indexa = 0;
			int endindexa = 0;
		...
		if (indextype > 0) {
			...
			int endAre = contmp.indexOf("</span>");
			if (endAre >= 0 && indexar >= 0 && endAre > indexar) {
				String sourceare = contmp.substring(indexar, endAre); // sourceare是国家。
				String con = "http://www.youku.com/v_olist/c_96_g_";
				String endcon = ".html";
				int indexa = 0;
				int endindexa = 0;
		...
		int indexact = source.indexOf("<label>主演:</label>");
		if (indexact > 0) {
			String contmp1 = source.substring(indexact);
			int indexar1 = contmp1.indexOf("<label>主演:</label>");
			int endAre1 = contmp1.indexOf("</span>");
			if (indexar1 >= 0 && endAre1 >= 0) {
		...
		int indexdura = source.indexOf("<label>时长:</label>");
		if (indexdura > 0) {
		...
		quals = new String[] { "inforowkey", "year", "website", "name",
				"pictureURL", "othername", "time", "ytime", "lan", "area",
				"type", "director", "mainactor", "actorlist", "duration",
				"price", "category", "summarize", "rewards", "url" };
		values = new String[] { urlkey, movieyear, "yk", moviename, jpgurl,
				alias, Time, YTime, Lan, Are, Type, Director, Actor, Actorprod,
				dura, price, "movie", detail, award, url };
		try {
		...
		quals = new String[] { "inforowkey", "year", "website", "flag",
				"timestamp", "name", "score", "doubanscore", "updown",
				"sumplaycount", "collect", "comment", "todayplaycount",
				"category","free" };
		values = new String[] { urlkey, movieyear, "yk", "n", timestampkey,
				moviename, score, douscore, dingcai, sum, shou, comment,
				incresum, "movie",free+"" };
		try {
